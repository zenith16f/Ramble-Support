...
import { UpdatePasswordProps } from "@/interfaces/userInterfaces";
import { signOut } from "next-auth/react";
import { useForm } from "react-hook-form";
import { toast } from "sonner";

const PasswordConfirm = ({ id, newPassword }: UpdatePasswordProps) => {
  //: React Hook Form
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const passwordRes = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/password/${id[0]}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    const passwordResponse = await passwordRes.json();

    if (passwordResponse.error) {
      toast.error(passwordResponse.error);
      return;
    }
