...
import { UpdatePersonaProps } from "@/interfaces/userInterfaces";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { toast } from "sonner";

const UpdateConfirm = ({ id, name, lastname }: UpdatePersonaProps) => {
  //: Use Router
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const updatePersona = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/persona/${id[0]}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    const personaResponse = await updatePersona.json();

    if (personaResponse.error) {
      toast.error(personaResponse.message, {
        duration: 2500,
        style: { width: "600px" },
      });
      return;
